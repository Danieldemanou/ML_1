...
def initNetwork(nn_arch, act_func_name):
    ...
    for i in range(np.size(nn_arch)-1):
        w = 0.01*np.random.rand(nn_arch[i+1],nn_arch[i])
        W.append(w)
...
def backward(error,W,Yp):
    ...
    gradB = [error]
    ## petite difficulté avec les indice de la boucle cr il faut commprendre ici que W a un indice d'avance sur Yp 
    ## petite astuce utilisé pour avoir la liste dans un ordre inversé 
    #####################
    # TO BE COMPLETED
    #j = 1 
    #for i in range(len(W)-1):
        #error = np.dot(W[j].T,error) * Yp[i]
        #error = np.dot(np.dot(W[j].T,error), Yp[j].T)
        #j = i 
        #gradB = [error] + gradB
    ...
    #return gradB
    #gradB = [error]
    # Next we're gonna add gradb_layer2
    j = len(W)
    
    for i in reversed(range(1, j)): 
        gradb = Yp[i-1] *  W[i].T.dot(gradB[0])
        gradB.insert(0, gradb)
        #gradB = [error] + gradB
    return gradB
...
def update(eta, batch_size, W, B, gradB, Y):
    ...
    for i in range(len(W)):
        if i == len(W)-1 :
            ...
            index = i + 1
        #grad_w = np.sum(gradB[index].dot(Y[i].T)).reshape(-1,1)/batch_size
        #grad_w = np.sum(gradB[i].dot(Y[i].T)).reshape(-1,1)/batch_size
        grad_w = gradB[i].dot(Y[i].T)/batch_size
        grad_b = np.sum(gradB[index], axis = 1).reshape(-1,1)/batch_size
        W[i] -= eta * grad_w
        B[index] -= eta * grad_b
...
def sigmoid(z):
    ...
    yp = y * (1-y)
    #yp = np.dot(y,(1-y).T)
    #print("shape of yp" +str(yp.shape))
...
def parseArgs():
    ...
    parser.add_argument('--arch', help='Architecture of the hidden layers', default=[128,64,32,16], nargs='+', type=int)
    #parser.add_argument('--n_c', help='Architecture of the hidden layers', default=[100], nargs='+', type=int)
    parser.add_argument('--act_func', help='Activation function name', default="sigmoid", type=str)
    #parser.add_argument('--act_func', help='Activation function name', default="ReLU", type=str)
...
def parseArgs_ipython(**kwarg):
    ...
    class Arguments():
        ...
        def __init__(self, arch = [128,64,32,16], act_func = "sigmoid", batch_size = 500, eta = .01, n_epoch = 100):
